...
def infer_face_pairing(
    ...
    half_face_self_unordered = (
        embedding[0],
        embedding[cusp_edge[0]],
        embedding[cusp_edge[1]],
    )
    half_face_other_unordered = (
        other_embedding[0],
        other_embedding[other_cusp_edge[0]],
        other_embedding[other_cusp_edge[1]],
    )

    # order the pairing
    # TODO: make a normalize function for this, like in edge case
    sort_indices = [
        i for i, _ in sorted(enumerate(half_face_self_unordered), key=lambda x: x[1])
    ]
    ...
    return (
        manifold_cell,
        half_face_self_ordered,
        other_manifold_cell,
        half_face_other_ordered,
    )
...
def get_face_pairing(embeddings, cusp_pairing):
    ...
    for i in (0, 1, 2):
        half_face_a.append(source_embedding.map.get(source_domain[i]))
        half_face_b.append(target_embedding.map.get(target_domain[i]))
