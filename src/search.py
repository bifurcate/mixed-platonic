...
def search(env_path, debug=False):
  ...
  if not (env_path.exists() and env_path.is_dir()):
    search_logger.error(f"search environment '{env_path.name}' does not exist, use generate to create")
  
  state = read_state(env_path)
  if state != 'init':
    search_logger.error(f"search environment '{env_path.name}' not in init state, skipping execution")
    return
  ...
  log_file_handler.setLevel(logging.DEBUG)  # You can have different levels per handler
