...
def draw_finger_cell(
    ...
    if embedding is None:
        ...
        draw_poly_finger_offset(
            ctx, (cx, cy), verts, embedding_labels, color_map, color_idx
        )


def draw_finger(ctx, construction, finger_pattern, finger_idx, color_map):
    for knuckle_idx in range(3):
        draw_finger_cell(
            ctx, construction, finger_pattern, finger_idx, knuckle_idx, color_map
        )
...
def draw_stack(finger_pattern, construction, output_filename):
    ...
    font_matrix = cairo.Matrix(xx=0.15, yx=0, xy=0, yy=-0.15, x0=0, y0=0)
