...
def get_regina_perm_data(
    manifold_face_pairing: ManifoldFacePairing,
) -> tuple[int, regina.Perm4]:
    ...
    perm_input = tuple(
        x for pair in zip(r_src_face_spec, r_tgt_face_spec) for x in pair
    )
...
def get_gluing_data(
    manifold_face_pairing: ManifoldFacePairing, num_tets: int, num_octs: int
) -> tuple[int, int, regina.Perm4]:
    src_tet_idx = get_regina_tet_idx(
        manifold_face_pairing.half_face_src, num_tets, num_octs
    )
    tgt_tet_idx = get_regina_tet_idx(
        manifold_face_pairing.half_face_tgt, num_tets, num_octs
    )
...
def glue_oct_internal(triangulation: regina.Triangulation3, base_idx: int):
    ...
    triangulation.tetrahedron(base_idx + 0).join(
        1, triangulation.tetrahedron(base_idx + 1), identity_perm
    )
    triangulation.tetrahedron(base_idx + 1).join(
        3, triangulation.tetrahedron(base_idx + 2), identity_perm
    )
    triangulation.tetrahedron(base_idx + 2).join(
        1, triangulation.tetrahedron(base_idx + 3), identity_perm
    )
    triangulation.tetrahedron(base_idx + 3).join(
        3, triangulation.tetrahedron(base_idx + 0), identity_perm
    )

    # glue up the bottom hemisphere
    triangulation.tetrahedron(base_idx + 4).join(
        1, triangulation.tetrahedron(base_idx + 5), identity_perm
    )
    triangulation.tetrahedron(base_idx + 5).join(
        3, triangulation.tetrahedron(base_idx + 6), identity_perm
    )
    triangulation.tetrahedron(base_idx + 6).join(
        1, triangulation.tetrahedron(base_idx + 7), identity_perm
    )
    triangulation.tetrahedron(base_idx + 7).join(
        3, triangulation.tetrahedron(base_idx + 4), identity_perm
    )

    # glue the hemipheres together
    triangulation.tetrahedron(base_idx + 0).join(
        2, triangulation.tetrahedron(base_idx + 4), identity_perm
    )
    triangulation.tetrahedron(base_idx + 1).join(
        2, triangulation.tetrahedron(base_idx + 5), identity_perm
    )
    triangulation.tetrahedron(base_idx + 2).join(
        2, triangulation.tetrahedron(base_idx + 6), identity_perm
    )
    triangulation.tetrahedron(base_idx + 3).join(
        2, triangulation.tetrahedron(base_idx + 7), identity_perm
    )


def to_regina_triangulation(
    manifold_celluation: ManifoldCellulation, num_tets: int, num_octs: int
    ...
    for i in range(num_tets):
        ...
        for p in pairings.values():
            src_tet_idx, tgt_tet_idx, r_face, perm = get_gluing_data(
                p, num_tets, num_octs
            )
            # we need to do this because each gluing is attempted twice, this will be fixed when
            # ManifoldFacePairing is refactored
            try:
                triangulation.tetrahedron(src_tet_idx).join(
                    r_face, triangulation.tetrahedron(tgt_tet_idx), perm
                )
